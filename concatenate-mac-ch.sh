...
echo "Duplicating main_ch.css..."
cp styles/chinese/main_ch.css temp.css

# Scrap the @import rules
echo "Removing @imports..."
sed -i ".bak" "/^@import.*\.css.*$/d" temp.css || error "@import removal failed"

# Append this file to root.css
echo "Prepending root_ch.css..."
cat styles/chinese/root_ch.css temp.css > temp && mv temp temp.css
...
cp temp.css stable/styles/chinese/black-highlighter_ch.css
...
mv temp.css stable/styles/chinese/black-highlighter_ch.min.css
echo "Created black-highlighter_ch.min.css"
