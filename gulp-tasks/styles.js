...
function addSupports() {
  return gulp
    ...
    .pipe(map((file, cb) => {
      var fileContents = file.contents.toString();
      fileContents = fileContents.replace("@charset \"utf-8\";", "@charset \"utf-8\";@import url(\"https://use.typekit.net/rit0pba.css\");@import url(\"https://fonts.googleapis.com/css?family=Poppins:600,800|Space+Mono:400,400i,700,700i&display=swap&subset=latin-ext\");@supports(--css:variables){");
      fileContents = fileContents + "}";
      file.contents = Buffer.from(fileContents);
      cb(null,file);
    }))
